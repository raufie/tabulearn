...
import numpy as np
class NStepOffpolicy(MethodBase):
    """
        This is a NStepOffPolicy Control with importance sampling
    ...
    def _pi(self, S, Q, eps):
        ...
        return probs
    def train(self, n_episodes:int, step_size:float, gamma = 1.0, eps = 0.2, behavior_eps=0.3, callback = None , callback_per_episode = 500, n = 5):
        steps_per_episode = []
        Rs = []
        for episode in tqdm(range(n_episodes)):    
            ...
            reward_sum = 0
            error = 0
            # actions are chosen with off policy
            ...
            while Tau != T - 1:
                ...
                if Tau >= 0:
                    ...
                    p = np.prod([self._pi(S[k%(n+1)], self.Q, eps)[A[k%(n+1)]]/self._pi(S[k%(n+1)], self.Q, behavior_eps)[A[k%(n+1)]] for k in range(Tau+1, min(Tau+n-1, T-1))])
                    self.Q[S[Tau%(n+1)], A[Tau%(n+1)]]+= step_size*p*(G - self.Q[S[Tau%(n+1)], A[Tau%(n+1)]])
                t+=1
